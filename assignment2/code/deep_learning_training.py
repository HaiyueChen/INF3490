...
imax = np.concatenate(
    (movements.max(axis=0) * np.ones(
        (1, 41)), np.abs(movements.min(axis=0) * np.ones((1, 41)))),
    axis=0).max(axis=0)
...
deep = deep_learning(train, train_targets, hidden_1, hidden_2)

end_time = time.time()
# Run training:
deep.earlystopping(train, train_targets, valid, valid_targets)
